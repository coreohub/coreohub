...
0. **Teto por participante no Começo (discutido 2026-09-08, não implementado)** — avaliação de mercado (5 festivais reais + concorrentes por-pessoa) recomendou considerar um teto de R$4-5/participante no Começo, no mesmo espírito do teto de 4,5% do Escala, pra evitar que a comissão de 10% dispare 3-4x acima da concorrência em eventos de ticket médio alto (>R$40/pessoa). **Não é só mudar a página**: o Começo hoje cobra 100% via split contínuo em tempo real, sem nenhum mecanismo de acerto de fechamento — um teto por participante só pode ser aplicado sabendo o número real de participantes, que só existe no fechamento do evento (mesma razão pela qual o Escala precisou da Fase 3 de settlement). Implementar exigiria construir pro Começo uma infraestrutura equivalente à do Escala (migration + RPC de preview + edge function de settlement + branch no webhook) — projeto do mesmo porte do que foi feito em 2026-09-04, não uma mudança de copy. Decisão de produto (valor do teto, se R$4 ou R$5) ainda em aberto; decisão de implementar tecnicamente nem chegou a ser tomada.
