...
**🟢 Pendência — auditar outras tabelas por policy "Acesso Total X" fantasma (2026-07-17, achado #3 em 2026-07-19)**: 3ª ocorrência da MESMA classe de bug (`registrations` 2026-05-22, `events` 2026-07-17, `profiles` 2026-07-19 — essa já corrigida, era `any_authenticated_updates_profile_safe`). Uma varredura full (`SELECT * FROM pg_policies WHERE qual='true' AND cmd IN ('UPDATE','ALL')`) em TODO o schema — não só tabela por tabela quando uma feature força a olhar — fecharia isso de vez em vez de continuar achando 1 de cada vez. Tabelas ainda não auditadas dessa forma: `audience_tickets`, `workshop_registrations`, `coupons`, `judges`, `team_invites`, etc. — algumas já tiveram trigger de proteção de COLUNAS aplicado em [[auditoria_rls_protect_triggers_shipado]], mas isso não garante que não exista também uma policy de acesso total sobrando.
