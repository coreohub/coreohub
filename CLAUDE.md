...
**🟧 Workshop Pass (commit `84d7796`, feito antes desta sessão de 2026-06-27) — finalizar deploy**: feature de "pacote fixo de N workshops" (Day Pass/Full Pass) foi codada (migration `20260628_workshop_passes.sql` + edge functions `create-workshop-pass-registration` + `detect-workshop-pass-combo` + branch `WSP:` no `asaas-webhook` + templates novos no `send-email` + UI em `WorkshopsManagement.tsx`/`PublicEventPage.tsx`/`CheckoutWorkshopPass.tsx`), mas o próprio commit já avisava "pendente aplicar migration + deploy". Na sessão de 2026-06-27 o `asaas-webhook` foi redeployado por outro motivo (fix de rebaixamento de pagamento) — isso subiu o código do branch `WSP:` também, mas **a migration `20260628_workshop_passes.sql` ainda não foi aplicada em prod, e as 2 edge functions novas (`create-workshop-pass-registration`, `detect-workshop-pass-combo`) ainda não foram deployadas**, nem o `send-email` redeployado com os templates novos. Sem risco imediato (branch `WSP:` fica inerte sem a migration — nenhum checkout consegue gerar esse externalReference), mas a feature não está funcional pro produtor ainda. Falta: `supabase db query --linked --file supabase/migrations/20260628_workshop_passes.sql` + `supabase functions deploy create-workshop-pass-registration` + `supabase functions deploy detect-workshop-pass-combo` + `supabase functions deploy send-email`.
