...
- **Armadilha operacional real**: os bugs 2 e 3 pareciam "voltar sozinhos" depois de corrigidos — na verdade era a aba de Configurações aberta desde antes de cada deploy, rodando o JS antigo do PWA (`registerType: 'autoUpdate'`) e revertendo o fix ao salvar. Backlog aberto (não implementado): trocar por prompt de update que força reload, resolveria a causa raiz.
...
- **Validação**: `npm run lint` + `npm run build` + 92 testes limpos em cada um dos 4 commits.
...
**🚨 COBRAR os R$250 da Lorrayne (taxa fixa do plano Essencial, 2026-09-13)**: ao corrigir o gap de comissão de workshop/ingresso, o `billing_plan` do evento dela (Vicenza Dance Camp 2027) foi movido pra `essencial` direto via SQL — isso contornou a cobrança adiantada de R$250 que o plano exige (`billing_plan_fixed_fee_paid_at` continua `NULL` no banco). Ela está recebendo 5% de comissão sem ter pago o fixo. Combinado: mandar WhatsApp avisando ela primeiro (comissão caiu + cobrança sai "amanhã", 2026-09-14), só depois montar a cobrança de verdade — sem fluxo pronto pra isso hoje (o gate normal `create-plan-fixed-fee-payment` só dispara enquanto `billing_plan='comeco'`, que já não é mais o caso dela; vai precisar adaptar). Detalhes em `memory/billing_plan_sync_workshop_audience_shipado.md`.
...
**🟧 Pendência de validação — cron `revoke-expired-team-access` nunca disparou de verdade (2026-07-19)**: cron diário criado + agendado (`events.equipe_access_days_after`, default 30d, contado de `configuracoes.data_evento`), mas nenhum evento real ainda passou do grace period — só testado via chamada direta simulando o efeito. Validar quando o Usualdance Festival (encerrado 11/07) completar 30 dias (~10/08/2026): conferir se a equipe perde acesso sozinha e recebe a notificação in-app.
...
**🟠 Pendência de verificação recorrente — Egress do Supabase estourado (checar semanalmente até 12/08/2026)**: org bateu 801% da cota de Egress do plano Free (40GB de 5GB no ciclo 05/07-05/08/2026) — carência até **12/08/2026**, depois disso a Fair Use Policy passa a restringir o projeto (requests podem voltar 402). Storage Size está OK (68%), o problema é banda de saída (replay de trilha/narração), não espaço ocupado. Causa provável corrigida em 2026-07-15 (commit `ea75ff3`): `cacheControl` de trilha/narração subiu de 1h pra 1 ano nos 3 pontos de upload (`InscricaoWizard.tsx`, `CentralDeMidia.tsx`, `generate-narration/index.ts`) — mas só vale pra uploads **novos** a partir dessa data, não retroage pros arquivos já existentes. Egress muda devagar (ciclo mensal) e o pico provável foi o próprio Usualdance (11/07, já passou) — checagem diária/a-cada-3-dias só mostra ruído. **Verificar 1x por semana** (dá pra fazer 3-4 checagens até 12/08): Dashboard → Usage (`https://supabase.com/dashboard/org/nkfhcqcgfraolffbiusg/usage`) → conferir se "Egress" está caindo em direção aos 5GB. Se não cair o suficiente, avaliar upgrade de plano (`manage your plan` no aviso) ou investigar outra fonte de egress (imagens de capa, PDFs de certificado, API do banco).
