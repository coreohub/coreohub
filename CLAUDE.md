...
- **Carrinho de ingressos multi-tipo na vitrine** — gap identificado 2026-05-28 (noite). Hoje comprador escolhe 1 tipo (Inteira OU Meia OU Solidária) e checkouta. Sympla/Eventbrite/Ticketmaster aceitam misturar tipos no mesmo carrinho — gap vs concorrência. Plano técnico fechado em [[backlog-carrinho-ingressos-multi-tipo]]: RPC v2 com array de items, edge function aceita items[], vitrine ganha +/− por tipo + sticky bar com total, checkout vira `/checkout-ingresso/<slug>` (sem ticketTypeIdx) com resumo editável inline. Reusa `group_id` existente (família semântica). ~5-6h. Trigger: produtor reclamar que comprador desiste no 2º checkout OU primeiro festival grande com ingressos online.
