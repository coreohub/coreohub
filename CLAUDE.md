...
**🟧 Pendência de validação — dia do Usualdance Festival, 11/07 (2026-07-10)**: sessão do dia anterior shipou 4 coisas que só serão exercitadas de verdade amanhã, no evento real: (1) Troféu Voto Popular automático no Telão — só testado via reprodução HTTP direta (confirmando que a chamada chega no código), nunca com uma urna de verdade fechada e vencedor real; (2) trava server-side de `deliberation_status==='LIBERADO'` em `submit-deliberation`/`remove-marcacao` — lógica coberta por código, sem produtor tentando editar deliberação já liberada em prod; (3) Súmula geral em PDF (`JudgesManagement.tsx`) nunca impressa de verdade como contingência de papel; (4) acesso único do jurado via QR `?code=` — só testado por leitura de código, sem QR físico escaneado num aparelho real. Acompanhar o Hemer no dia e resolver na hora se algo quebrar.
