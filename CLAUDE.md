...
**🟢 Pendência — auditar outras tabelas por policy "Acesso Total X" fantasma (2026-07-17)**: 2ª vez que uma policy `USING(true) WITH CHECK(true)` sobrevive em produção sem ninguém notar até uma feature nova forçar a investigação (`registrations` em 2026-05-22, `events` agora). Vale grep em `pg_policies` por `qual = 'true'` nas tabelas restantes (`audience_tickets`, `workshop_registrations`, `coupons`, `judges`, etc. — algumas já tiveram trigger de proteção de COLUNAS aplicado em [[auditoria_rls_protect_triggers_shipado]], mas isso não garante que não exista também uma policy de acesso total sobrando).
