...
**🆕 P1 — PWA: trocar `autoUpdate` silencioso por prompt "Nova versão disponível" (2026-09-15)**: `registerType: 'autoUpdate'` faz aba aberta continuar rodando JS velho até fechar/recarregar — se o produtor salva algo nessa aba depois de um deploy, reverte o fix silenciosamente. **Já aconteceu 2x na mesma sessão** (2026-09-15, fix de preço progressivo revertido pela aba de Configurações do Ticko aberta desde antes do deploy) — trigger de implementação já foi disparado. Plano pronto (trocar `registerType` pra `'prompt'` + hook `useRegisterSW` de `virtual:pwa-register/react` + banner "Nova versão disponível — Atualizar"), ~1h–1h30, maior parte é validar com 2 deploys reais em sequência. Detalhes em `memory/backlog_pwa_update_prompt_forcar_reload.md`.
