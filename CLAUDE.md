...
**🟧 Pendência de validação — checagem de áudio pré-entrada do júri em hardware físico real (2026-07-16)**: `JudgeMicCheck.tsx` (ver Histórico recente) só foi validado com mic fake do Chromium em dev server — nunca com tablet físico real num evento ao vivo. Testar no próximo evento: equipe configurando vários tablets em sequência, "Continuar sem áudio" acessível, device escolhido realmente sendo usado na gravação.
...
- **Modo claro como padrão (aguardando decisão do produtor)** — auditoria completa 2026-08-08: 66 rotas capturadas em claro (desktop+mobile via Playwright, `--theme=light` novo em `scripts/screenshot.mjs`), revisadas por 8 leituras independentes. **Achado que muda o escopo**: não é 1 decisão, são 2 — o app interno (produtor/inscrito/admin) já tem modo claro construído e quase pronto; a vitrine pública (`LandingPage.tsx`, `LandingGoverno.tsx`, `PublicEventPage.tsx`, `PublicWorkshopPage.tsx`, `Festivais.tsx`, `Leaderboard.tsx`, `TermosDeUso.tsx`, `PoliticaDePrivacidade.tsx`, `EntrarJuri.tsx`) tem `bg-black`/`bg-slate-950`/`bg-[#050505]` hardcoded sem nenhuma variante clara — virar padrão ali é desenhar do zero, não corrigir contraste. **5 bugs bloqueadores no app interno** antes de qualquer virada: `AIAnalysis.tsx` (tela inteira presa ao dark), config de Banca de Jurados `/equipe-jurados-config` (cards + seção "COMPETÊNCIAS" ilegíveis), painel do Telão `/telao-palco` (texto auxiliar do código quase invisível), `BattleConfig.tsx` (cards cinza esquecidos do dark), `RegistrationGradeConfig.tsx`/`/event-config` (título cru + overflow mobile, já stub conhecido). **1 padrão sistêmico** pra corrigir 1x só: botões rosa-pastel+texto branco com baixo contraste em claro, repetido em ≥5 lugares (Transferir agora, Continuar do wizard, Vender no Balcão, Validar certificado, Indisponível de workshop) + ícones de power/toggle fracos demais (Cupons, Avisos). **Telas que devem ficar escuras sempre, independente do padrão**: Marcação de Palco, preview "Ao vivo" do Telão, Telão Display (`/telao`), Battle Arena Live — ambiente físico de pouca luz ou réplica de LED. **Recomendação registrada**: Opção B — claro como padrão só no app interno, vitrine pública/palco continuam dark (reforça identidade "festival", e não têm modo claro construído hoje). Relatório completo com screenshots e opções A/B/C em [[modo_claro_auditoria_2026_08_08]]. **Achados fora do pedido, não relacionados a tema** (reportados junto): tabela de Cupons cortada no mobile sem indicador de scroll; bio do professor duplicada na vitrine de workshop (`workshop/jazz-funk-com-jonathan-lupe`); ícone sobrepondo título no Battle Arena mobile; 35 console errors em `/manage-schedule` mobile (não visível no layout, precisa investigação isolada); 1 console error cada em `/evento/:slug` e `/ingressos`.
