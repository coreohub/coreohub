-- Telão exibe o número "do dia" (ordem_apresentacao_dia) quando o bloco
-- ao vivo tem data — evento multi-dia (separar numeração por dia no
-- Cronograma, sessão 2026-08-03). Cai pro número global quando o bloco não
-- tem data (evento de 1 dia só, comportamento idêntico ao anterior).
--
-- CREATE OR REPLACE integral de get_telao_state (v2, 20260708) — única
-- mudança é o SELECT de v_reg (+ordem_apresentacao_dia) e o campo 'numero'
-- do jsonb de retorno.
...
BEGIN
  ...
  SELECT id, nome_coreografia, estudio, event_data, categoria, estilo_danca,
         ordem_apresentacao, ordem_apresentacao_dia, tipo_apresentacao
    INTO v_reg
    FROM registrations
  ...
  RETURN jsonb_build_object(
    ...
    'coreografia', jsonb_build_object(
      'numero', coalesce(v_reg.ordem_apresentacao_dia, v_reg.ordem_apresentacao), 'nome', v_reg.nome_coreografia, 'estudio', v_estudio,
