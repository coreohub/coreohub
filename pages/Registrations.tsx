...
import {
  ...
  BarChart3, MessageCircle, Link2, ShieldCheck, Check,
} from 'lucide-react';
...
const Registrations = () => {
  ...
  const handleSaveEdit = async (regId: string) => {
    ...
    try {
      ...
      patch.is_pcd = !!editValues.is_pcd;
  ...
  useEffect(() => {
    ...
    if (pcdOnly) result = result.filter(reg => reg.is_pcd);
  ...
  }, [searchTerm, paymentFilter, modalidadeFilter, categoriaFilter, estiloFilter, inscritoFilter, tipoApresentacaoFilter, dateFilter, quickAlert, pcdOnly, registrations, elencoById]);
  ...
  useEffect(() => {
    setPage(1);
  }, [searchTerm, paymentFilter, modalidadeFilter, categoriaFilter, estiloFilter, inscritoFilter, tipoApresentacaoFilter, dateFilter, quickAlert, pcdOnly, sortBy, sortDir]);
  ...
  const handleBulkExportCSV = () => {
    ...
    const headers = ['Data', 'Coreografia', 'Modalidade', 'Tipo', 'Estúdio', 'Cidade Estúdio', 'UF Estúdio', 'Categoria', 'Estilo', 'PCD', 'Inscrito', 'Email', 'WhatsApp', 'Status Inscrição', 'Status Pagamento', 'Valor (R$)', 'Bailarinos', 'Equipe Técnica'];
    const escape = (v: any) => {
    ...
    const rows = selectedRegs.map((r: any) => {
      ...
      return [
        ...
        r.categoria ?? '', r.estilo_danca ?? '', r.is_pcd ? 'Sim' : 'Não',
  ...
  const alertCounts = useMemo(() => {
    ...
    return { vencendo, noTrilha, incompletos, pcd };
  ...
  const handleExportCSV = () => {
    ...
    const headers = [
      ...
      'Categoria', 'Estilo', 'PCD', 'Inscrito', 'Email', 'WhatsApp',
    ...
    <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in duration-700 pb-20">
      ...
      {tab === 'LIST' ? (
        <div className="space-y-6">
          ...
          {/* PCD (eixo opcional de dança inclusiva). Não é alerta/problema —
              chip informativo separado do banner "Atenção" (amber), estilo
              sky pra bater com o badge "PCD" nas linhas/cards. */}
          {alertCounts.pcd > 0 && (
            <div className="flex items-center gap-2">
              <button
                onClick={() => setPcdOnly(v => !v)}
                className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all flex items-center gap-1.5 ${
                  pcdOnly
                    ? 'bg-sky-500 text-white border-sky-500'
                    : 'bg-white dark:bg-sky-500/10 text-sky-700 dark:text-sky-300 border-sky-200 dark:border-sky-500/30 hover:bg-sky-100 dark:hover:bg-sky-500/20'
                }`}
                title="Mostrar só inscrições marcadas como dança inclusiva (PCD)"
              >
                <ShieldCheck size={11} /> {alertCounts.pcd} PCD
              </button>
              {pcdOnly && (
                <button
                  onClick={() => setPcdOnly(false)}
                  className="px-2 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest text-sky-700 dark:text-sky-400 hover:bg-sky-100 dark:hover:bg-sky-500/20 transition-all flex items-center gap-1"
                >
                  <X size={11} /> Limpar
                </button>
              )}
                ...
                <section>
                  ...
                  {editing ? (
                    <>
                      ...
                      <button
                        type="button"
                        onClick={() => setEditValues(p => ({ ...p, is_pcd: !p.is_pcd }))}
                        className={`mt-3 w-full flex items-center gap-2 p-3 rounded-xl border transition-all text-left ${
                          editValues.is_pcd
                      ...
                      >
                        <div className={`shrink-0 w-4 h-4 rounded border-2 flex items-center justify-center ${
                          editValues.is_pcd ? 'border-sky-500 bg-sky-500' : 'border-slate-300 dark:border-white/20'
                        }`}>
                          {editValues.is_pcd && <Check size={11} className="text-white" strokeWidth={3} />}
                        </div>
                        <span className={`text-[10px] font-black uppercase tracking-widest ${editValues.is_pcd ? 'text-sky-600 dark:text-sky-400' : 'text-slate-600 dark:text-slate-300'}`}>
                          Dança inclusiva (PCD)
                        </span>
                      </button>
                    </>
