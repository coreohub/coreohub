  ...
  const fetchData = async () => {
    ...
    try {
      const [{ data: regs, error }, { data: evt }] = await Promise.all([
        supabase
          .from('registrations')
          .select('id, nome_coreografia, estudio, categoria, modalidade, video_url, video_status, video_feedback, video_submitted_at, video_fee_status, profiles(full_name)')
              ...
              {filtered.map(reg => (
                <tr key={reg.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors group">
                  <td className="px-6 py-4">
                    ...
                    <p className="text-[9px] text-[#ff0068] font-bold uppercase tracking-widest">{reg.modalidade}</p>
                  </td>
