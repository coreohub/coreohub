...
import { getInviteByToken, markInviteUsed, type ProducerInvite } from '../services/inviteService';
import {
  Crown, Loader2, AlertCircle, CheckCircle, ArrowRight, Lock, Mail, User,
} from 'lucide-react';

const ProducerInviteLanding = () => {
  const { token } = useParams<{ token: string }>();
  const navigate = useNavigate();

  const [invite, setInvite]   = useState<ProducerInvite | null>(null);
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      // 2. Atualiza profile (alguns setups criam o profile via trigger; aqui garantimos os campos)
      await supabase
        .from('profiles')
        .upsert({
          id:        data.user.id,
      ...
      // 4. Redireciona para configuração de pagamento
      navigate('/account-settings?tab=Pagamentos&welcome=1');
    } catch (e: any) {
      setFormError(e.message?.includes('already') ? 'Já existe uma conta com este e-mail. Faça login.' : e.message);
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="text-center space-y-2">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#e3ff0a]/10 border border-[#e3ff0a]/20">
            ...
            <span className="text-[9px] font-black text-[#a8b800] dark:text-[#e3ff0a] uppercase tracking-[0.3em]">Convite Exclusivo</span>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-white/10 rounded-3xl p-6 space-y-5 shadow-sm">
          ...
          <div className="text-[10px] text-center text-slate-400 leading-relaxed">
            Ao criar a conta você será direcionado para a configuração de recebimento (CPF/CNPJ + chave PIX) — apenas 2 campos.
          </div>
        </div>
...
export default ProducerInviteLanding;
