    ...
    return (
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-10 text-center">
          <p className="text-slate-400 text-sm font-bold uppercase tracking-widest">Crie um evento primeiro pra usar o telão.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-white/5 border border-white/10 rounded-3xl p-6 space-y-6">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className={`text-sm font-black uppercase tracking-widest ${ativo ? 'text-emerald-400' : 'text-slate-500'}`}>
          ...
          {code && (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all disabled:opacity-50 ${
                ativo ? 'bg-white/5 text-slate-300 hover:bg-white/10' : 'bg-emerald-500 text-white hover:bg-emerald-600'
              }`}
            >
        ...
        {!code ? (
          <div className="text-center py-6 space-y-4">
            <p className="text-slate-400 text-sm">Gere o código do telão pra começar. O operador do LED vai digitar ele em <b className="text-white">{origin.replace(/^https?:\/\//, '')}/telao</b>.</p>
            <button
              onClick={handleGenerate}
              disabled={busy}
              className="inline-flex items-center gap-2 px-6 py-3.5 bg-[#ff0068] hover:bg-[#d4005a] disabled:opacity-50 text-white rounded-2xl font-black text-sm uppercase tracking-widest transition-all"
            >
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              <button onClick={() => copy('link')} className="flex items-center justify-center gap-2 py-3 bg-white/5 hover:bg-white/10 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all">
                {copied === 'link' ? <Check size={13} className="text-emerald-400" /> : <Copy size={13} />} Copiar link
              </button>
              <button onClick={() => copy('code')} className="flex items-center justify-center gap-2 py-3 bg-white/5 hover:bg-white/10 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all">
                {copied === 'code' ? <Check size={13} className="text-emerald-400" /> : <Copy size={13} />} Copiar código
              </button>
              <a href={publicUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 py-3 bg-[#ff0068] hover:bg-[#d4005a] text-white rounded-xl font-black text-[10px] uppercase tracking-widest transition-all">
                <ExternalLink size={13} /> Abrir telão
              </a>
              <a href={`${publicUrl}?teste=1`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 py-3 bg-white/5 hover:bg-white/10 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all">
            ...
            <button
              onClick={handleGenerate}
              disabled={busy}
              className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-white transition-all disabled:opacity-50"
            >
            ...
            {ativo && (
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Preview ao vivo</p>
      ...
      <div className="bg-white/5 border border-white/10 rounded-3xl p-6 space-y-3">
        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Como usar no dia do evento</p>
        <ol className="text-sm text-slate-300 space-y-2 list-decimal pl-5">
          <li>No computador ligado ao LED/projetor, abra <b className="text-white">{origin.replace(/^https?:\/\//, '')}/telao</b> e digite o código acima. Pressione <b className="text-white">F11</b> pra tela cheia.</li>
          <li>Ao clicar <b className="text-white">Iniciar</b> numa coreografia no Cronograma, o telão mostra "aguardando" com os jurados daquela apresentação.</li>
          <li>Quando o <b className="text-white">último jurado fecha a nota</b>, a média aparece sozinha. O operador corta pra essa tela no aplauso.</li>
          <li>Use <b className="text-white">Testar telão</b> antes das portas abrirem pra ajustar o tamanho da janela ao painel (ex: 6×2 m).</li>
        </ol>
      </div>
    </div>
  );
